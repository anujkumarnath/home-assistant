...
class AcCycleFlowCard extends HTMLElement {
  ...
  _build() {
    this.innerHTML = `
      <ha-card>
        <style>
          ...
          .ac-switch-icon { --mdc-icon-size: 15px; }
        ...
        <div class="ac-card">
          <div class="ac-header">
            <div class="ac-title">${this._config.title}</div>
            ${
          ...
          <div class="ac-flow-row">
            <div class="ac-node plug1">
              <div class="ac-node-ring-wrap" data-toggle-switch="plug1">
                ...
                <div class="ac-node-circle"><ha-icon icon="${this._config.plug1_icon}"></ha-icon></div>
                <div class="ac-node-badge"><ha-icon icon="mdi:cloud-off-outline"></ha-icon></div>
              </div>
              <div class="ac-node-label">${this._config.plug1_name}<span class="ac-node-time"></span></div>
              <div class="ac-node-power"><ha-icon icon="mdi:lightning-bolt"></ha-icon><span></span></div>
              ${this._config.plug1_participate_entity ? `<button class="ac-participate" data-toggle-participate="plug1" type="button"><span class="dot"></span><span class="txt"></span></button>` : ""}
            </div>
            <div class="ac-arrows">
            ...
            <div class="ac-node plug2">
              <div class="ac-node-ring-wrap" data-toggle-switch="plug2">
                ...
                <div class="ac-node-circle"><ha-icon icon="${this._config.plug2_icon}"></ha-icon></div>
                <div class="ac-node-badge"><ha-icon icon="mdi:cloud-off-outline"></ha-icon></div>
              </div>
              <div class="ac-node-label">${this._config.plug2_name}<span class="ac-node-time"></span></div>
              <div class="ac-node-power"><ha-icon icon="mdi:lightning-bolt"></ha-icon><span></span></div>
              ${this._config.plug2_participate_entity ? `<button class="ac-participate" data-toggle-participate="plug2" type="button"><span class="dot"></span><span class="txt"></span></button>` : ""}
            </div>
          </div>

          <div class="ac-phase"><span class="dot2"></span><span class="phase-text">—</span></div>

          ${this._config.auto_cycle_entity || this._config.max_runtime_entity ? `<div class="ac-divider"></div>` : ""}
          <div class="ac-footer">
            ${
              this._config.auto_cycle_entity
            ...
            ${
              this._config.max_runtime_entity
                ? `<div class="ac-footer-row">
    ...
    this._els = {
      card: this.querySelector(".ac-card"),
      node1: this.querySelector(".ac-node.plug1"),
      node2: this.querySelector(".ac-node.plug2"),
      ringWrap1: this.querySelector(".ac-node.plug1 .ac-node-ring-wrap"),
      ringWrap2: this.querySelector(".ac-node.plug2 .ac-node-ring-wrap"),
      ringProgress1: this.querySelector(".ac-node.plug1 .ac-ring-progress"),
      ringProgress2: this.querySelector(".ac-node.plug2 .ac-ring-progress"),
      time1: this.querySelector(".ac-node.plug1 .ac-node-time"),
      time2: this.querySelector(".ac-node.plug2 .ac-node-time"),
      power1: this.querySelector(".ac-node.plug1 .ac-node-power"),
      power2: this.querySelector(".ac-node.plug2 .ac-node-power"),
      participate1: this.querySelector('[data-toggle-participate="plug1"]'),
      participate2: this.querySelector('[data-toggle-participate="plug2"]'),
      arrowFwd: this.querySelector(".ac-arrow-path.fwd"),
      arrowRev: this.querySelector(".ac-arrow-path.rev"),
      markerFwd: this.querySelector(".ac-arrow-marker.fwd"),
      markerRev: this.querySelector(".ac-arrow-marker.rev"),
      phaseText: this.querySelector(".phase-text"),
      dot2: this.querySelector(".ac-phase .dot2"),
      awaySwitch: this.querySelector(".ac-switch.away"),
      autoCycleSwitch: this.querySelector(".ac-switch.autocycle"),
      runtimeValue: this.querySelector(".ac-stepper-value"),
    };

    for (const c of [this._els.ringProgress1, this._els.ringProgress2]) {
      c.style.strokeDasharray = `${RING_CIRCUMFERENCE} ${RING_CIRCUMFERENCE}`;
    }
  ...
  _update() {
    ...
    this._els.node1.classList.toggle("active", plug1On);
    this._els.node2.classList.toggle("active", plug2On);
    this._els.ringWrap1.classList.toggle("offline", plug1Offline);
    this._els.ringWrap2.classList.toggle("offline", plug2Offline);

    this._updateRing(this._els.ringProgress1, this._els.time1, this._config.plug1_timer);
    this._updateRing(this._els.ringProgress2, this._els.time2, this._config.plug2_timer);

    this._updatePower(this._els.power1, this._config.plug1_power_entity, this._config.plug1_current_entity, plug1On);
    this._updatePower(this._els.power2, this._config.plug2_power_entity, this._config.plug2_current_entity, plug2On);

    if (this._config.plug1_participate_entity) {
      this._updateParticipate(this._els.participate1, hass.states[this._config.plug1_participate_entity]?.state !== "off");
    }
    if (this._config.plug2_participate_entity) {
      this._updateParticipate(this._els.participate2, hass.states[this._config.plug2_participate_entity]?.state !== "off");
    }

    const switchingFwd = phase.startsWith("Switching: Plug 1");
    const switchingRev = phase.startsWith("Switching: Plug 2");
    this._els.arrowFwd.classList.toggle("flowing", switchingFwd);
    this._els.arrowRev.classList.toggle("flowing", switchingRev);
    ...
    this._els.markerFwd.style.fill = switchingFwd ? flowColor : dimColor;
    this._els.markerRev.style.fill = switchingRev ? flowColor : dimColor;

    const displayPhase = phase
      .replaceAll("Plug 1", this._config.plug1_name)
      .replaceAll("Plug 2", this._config.plug2_name);
    this._els.phaseText.textContent = displayPhase;
